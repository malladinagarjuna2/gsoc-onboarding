...
def PosUpdater(speed: SignallingRef[IO, Double], pos: SignallingRef[IO, Double]) =
    Stream.fixedRate[IO](updateMs.millis)
    .evalMap ( _ =>
        speed
    ...
    for 
        ...
        node <- div(
                ...
                if r then
                    div(
                        ...
                        span(
                            ...
                            button(
                                "Go!",
                                onClick --> (_.foreach(_ => vroom.get.flatMap(
                                    vro => speed.update(v => v + speedScale * countOs(vro))
                                )))
                            ),
                        )
                    )
            ...
            button(
                ...
                revealed
                .map(r => if r then "Put my car in the garage" else "Click to see my car!")
            )
        )
    yield node
